 ...
 * 
 * <p>
 * 封装游戏中的Executor，游戏中的Executor基本都是多生产者，单消费者(保证线程安全，不然就玩家身上所有的数据操作要加锁)。 
 * 在AMD a8-5600k APU with Radeom(tm)HD . 
 * Graphics 3.59HZ;win7 64位；8G(7.48G可用)测试：吞吐400w，在任务中只计数。
...
public class DisruptorExecutor implements Executor {
   ...
   * 构造函数.
   * 
   * Disruptor 从3.3版本开始, 去掉了自定义线程池的接口. 改成了线程工厂创建. 
   * 线程池创建的线程, 如果没有生产者的情况下线程会被挂起. 作者希望消费者的数量由创建的处理器数量决定, 而不是人为的指定
   * 此情况适用于多生产者多消费者情况
   * 
   * 对于线程工厂, 不应该包含业务, 业务校验完成后, 线程工厂产生线程, 这种方式不太认可. 所以这里使用disruptor
   * 依旧使用自定义线程池处理.
   * 
   * @param name 名称.
   */
